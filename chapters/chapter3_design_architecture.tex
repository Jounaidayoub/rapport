...
\textbf{Key benefits of Docker Compose in our system:}
\begin{itemize}
    \item \textbf{Service Isolation}: Each component (Kafka, Redis, Elasticsearch, etc.) runs in its own container, avoiding dependency conflicts.
    \item \textbf{Networking}: Docker Compose creates a dedicated network so containers can communicate using service names, simplifying service discovery and integration.
    \item \textbf{Dependency Management}: The \texttt{depends\_on} directive ensures services start in the correct order (e.g., Kafka waits for Zookeeper).
    \item \textbf{Port Mapping}: Exposes necessary ports for host access (e.g., 9092 for Kafka, 6379 for Redis, 9200 for Elasticsearch).
    \item \textbf{Scalability}: Easily scale services (like Celery workers) by adjusting the \texttt{docker-compose.yml} file.
    \item \textbf{Reproducibility}: The entire stack can be started or stopped with a single command (\texttt{docker-compose up/down}), ensuring consistent environments for all developers and deployments.
\end{itemize}
\begin{figure}[H]
    \centering
...
\end{figure}
\textbf{Example docker-compose.yml excerpt:}
\begin{minted}[fontsize=\footnotesize, frame=lines, label=docker-compose.yml]{yaml}
version: '3.8'
services:
  zookeeper:
  ...
  # Additional services (FastAPI, Celery, Data Generator) would be defined similarly
\end{minted}

\textbf{How it all fits together:}
\begin{itemize}
